...
export function PipelineStepper({ runId, states, status, generationOutcome }: Props) {
  ...
  return (
    <aside className="border-b border-white/10 bg-card/45 lg:sticky lg:top-16 lg:h-[calc(100dvh-4rem)] lg:border-b-0 lg:border-r lg:bg-card/25">
      <div className="h-full px-4 py-4 lg:px-5 lg:py-7">
        <div className="mb-4 hidden lg:block">
          <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Pipeline rail</div>
          <div className="mt-2 truncate font-mono text-xs text-foreground">RUN {shortRunId(runId)}</div>
        </div>
            ...
            return (
              <li key={s.id} className="flex shrink-0 items-center gap-1 lg:block">
                <Link
                  ...
                  params={{ runId }}
                  className={cn(
                    "group flex min-h-11 items-center gap-2.5 rounded-md border border-transparent px-3 py-2 text-sm transition",
                    active
