...
function CompareStage() {
  ...
  return (
    <div>
      ...
      <section className="glass-panel mb-6 rounded-xl p-4 sm:p-5" aria-labelledby="save-experiment-heading">
        <div className="mb-4">
          <div className="font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Capture a baseline</div>
          <h2 id="save-experiment-heading" className="mt-2 font-display text-xl font-semibold tracking-tight">Save this run as an experiment</h2>
          <p className="mt-1 max-w-2xl text-sm text-muted-foreground">Snapshots keep the configuration, model provenance, runtime, and selected caption together for a clean comparison.</p>
        </div>
        <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(170px,0.45fr)_auto] sm:items-end">
          <div><label htmlFor="experiment-label" className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Snapshot label</label><input id="experiment-label" value={label} onChange={(event) => setLabel(event.target.value)} placeholder="Hybrid baseline" className="mt-2 h-11 w-full rounded-md border border-white/15 bg-background px-3 text-sm outline-none transition focus-visible:ring-2 focus-visible:ring-ember" /></div>
          <label><span className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Caption style</span><select value={style} onChange={(event) => setStyle(event.target.value)} className="mt-2 h-11 w-full rounded-md border border-white/15 bg-background px-3 text-sm outline-none transition focus-visible:ring-2 focus-visible:ring-ember"><option value="formal">Formal</option><option value="sarcastic">Sarcastic</option><option value="humorous-tech">Humorous Tech</option><option value="humorous-non-tech">Humorous Non-Tech</option></select></label>
          <Button onClick={saveExperiment} disabled={busy || !hasCaptions} className="min-h-11 gap-2"><Save className="h-4 w-4" aria-hidden="true" /> Save Experiment</Button>
        </div>
      </section>

      {run.experiments.length < 2 ? (
        <div className="glass-panel rounded-xl border-dashed p-8 text-center sm:p-10"><Beaker className="mx-auto h-8 w-8 text-muted-foreground" aria-hidden="true" /><div className="mt-3 text-lg font-semibold">Save one more snapshot to compare.</div><p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">Generate captions, save Experiment A, change a supported setting, regenerate the affected stages, then save Experiment B.</p></div>
      ) : (
        <>
          <div className="mb-6 grid gap-3 sm:grid-cols-2"><label><span className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Experiment A</span><select value={leftId} onChange={(event) => setLeftId(event.target.value)} className="mt-2 h-11 w-full rounded-md border border-white/15 bg-background px-3 text-sm outline-none transition focus-visible:ring-2 focus-visible:ring-ember">{run.experiments.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label><label><span className="font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Experiment B</span><select value={rightId} onChange={(event) => setRightId(event.target.value)} className="mt-2 h-11 w-full rounded-md border border-white/15 bg-background px-3 text-sm outline-none transition focus-visible:ring-2 focus-visible:ring-ember">{run.experiments.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label></div>
...
function ComparisonView({ comparison }: { comparison: ExperimentComparison }) {
  ...
  return <div className="space-y-6"><div className="flex justify-end"><Button type="button" variant="outline" size="sm" className="min-h-11 gap-2" onClick={copySummary}>{copied ? <Check className="h-3.5 w-3.5" aria-hidden="true" /> : <Copy className="h-3.5 w-3.5" aria-hidden="true" />} {copied ? "Copied" : "Copy comparison summary"}</Button></div><div className="grid gap-6 lg:grid-cols-2"><ExperimentColumn side="A" experiment={comparison.left} /><ExperimentColumn side="B" experiment={comparison.right} /></div><div className="overflow-x-auto rounded-xl border border-border bg-card"><div className="flex items-center justify-between border-b border-border px-4 py-3"><div className="font-mono text-[11px] uppercase tracking-[0.22em] text-muted-foreground">configuration diff</div><ArrowLeftRight className="h-4 w-4 text-muted-foreground" aria-hidden="true" /></div><table className="w-full min-w-[640px] text-sm"><tbody className="divide-y divide-border">{fields.map(([label, field]) => <Row key={label} label={label} left={comparison.left[field as keyof Experiment]} right={comparison.right[field as keyof Experiment]} />)}</tbody></table></div></div>;
}
...
function ExperimentColumn({ side, experiment }: { side: "A" | "B"; experiment: Experiment }) {
  return <div className={cn("flex flex-col overflow-hidden rounded-xl border border-border bg-card", side === "B" && "bg-ember-soft/25")}><div className="flex items-start justify-between gap-4 border-b border-border/60 px-4 py-4 sm:px-5"><div className="min-w-0"><div className="font-mono text-[10px] uppercase tracking-[0.22em] text-muted-foreground">experiment {side}</div><div className="mt-1 truncate font-display text-2xl">{experiment.label}</div></div><span className="shrink-0 rounded-full border border-border bg-background px-2 py-0.5 font-mono text-[10px] uppercase tracking-[0.18em] text-muted-foreground">{experiment.style}</span></div><div className="px-4 py-5 sm:px-5"><div className="font-mono text-[10px] uppercase tracking-[0.22em] text-muted-foreground">caption</div><p className="mt-2 text-pretty text-base leading-relaxed">{experiment.caption}</p></div></div>;
}
