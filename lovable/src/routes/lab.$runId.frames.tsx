...
function FramesStage() {
  ...
  async function extract() {
    setBusy(true);
    const updated = await api.postFrames(runId, {
      method,
      totalFrames: total,
      anchorCount: anchors,
      highChangeCount: high,
      minSpacingSec: spacing,
      changeSensitivity: sens,
    });
    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.6fr]">
        <ConfigSection
          ...
          actions={
            <Button size="sm" className="gap-1.5" onClick={extract} disabled={busy}>
              {busy ? (
                <>
