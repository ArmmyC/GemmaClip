    ...
    <div className="min-h-screen bg-background">
          ...
          <div className="mt-10 grid gap-8 md:grid-cols-[1.15fr_1fr]">
            <div>
              ...
              <div className="mt-5 flex flex-wrap items-center gap-3">
                <Button size="lg" className="gap-2 rounded-full px-6" onClick={handleGenerate} disabled={busy}>
                  {busy ? "Uploading…" : "Generate captions"} <ArrowRight className="h-4 w-4" />
                </Button>
              </div>
              {error && <p role="alert" className="mt-3 text-sm text-destructive">{error}</p>}
              <p className="mt-4 font-mono text-[11px] uppercase tracking-[0.2em] text-muted-foreground">
                real backend · processing artifacts remain in configured run storage
              </p>
            </div>
      ...
      <footer className="border-t border-border">
        <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-3 px-6 py-8 md:flex-row md:items-center">
          ...
          <div className="font-mono text-[11px] uppercase tracking-[0.22em] text-muted-foreground">
            provider calls are made only by the backend
          </div>
        </div>
      </footer>
    </div>
